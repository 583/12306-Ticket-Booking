...
#            proxie = "{'http': 'http://127.0.0.1:8580'}"
            q_res = requests.get(url, headers=self.headers, verify=False)
#            print(q_res)
            html = q_res.json()
#            print(html)
...
#    date = input('请输入您要购票的乘车日期(例:2018-03-06):')
    ...
    while res != True:
        ...
        if booking_list[info_key] == True:
            ...
            break
        n += 1
        st = round(random.uniform(1, (7 - int(bkInfo.rank)) / 2) + random.uniform(0, len(booking_list)), 2) # linux服务器下延长访问时间，防止封禁ip
...
class BookingInfo(object):
    def __init__(self, bno, rank, username, password, from_station, to_station, dates, passengers_name, passengers_id_no, candidate_trains, candidate_seats, email, set_out, arrival, cddt_train_types):
        # 账号
        self.uuid = bno + '-' + dates
...
def task():
    ...
    for info_str in booking:
        if len(info_str) < 10:
            continue
        info = info_str.split('|')
        bkInfo = BookingInfo(info[0], info[1], info[2], info[3], info[4], info[5], info[6], info[7], info[8], info[9], info[10], info[11], info[12], info[13], info[14])
#        run(bkInfo)
...
#    time.sleep(300)
    client.connect(('39.96.21.111', 12306))
    schedule.every(5).seconds.do(keepalive)
    task()
